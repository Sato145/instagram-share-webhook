...
def extract_facebook_info(url, provided_username='', provided_caption='', provided_hashtags=''):
    """Facebook URLから投稿情報を取得"""
    ...
    try:
        ...
        print(f"Processing Facebook URL: {info.url}")
        
        # ユーザー名/ページ名を抽出（URLから）
        # パターン1: https://www.facebook.com/username/posts/xxxxx
        # パターン2: https://www.facebook.com/username/videos/xxxxx
        # パターン3: https://www.facebook.com/permalink.php?story_fbid=xxxxx&id=xxxxx
        username = None
        url_match = re.search(r'facebook\.com/([^/\?]+)(?:/posts|/videos|/photos)?', info.url)
        if url_match:
            potential_username = url_match.group(1)
            # 除外するパス
            if potential_username not in ['www', 'permalink.php', 'photo.php', 'watch', 'share']:
                username = potential_username
                print(f"✓ Extracted username from URL: {username}")
        
        # 投稿IDを抽出
        post_match = re.search(r'/posts/(\d+)', info.url)
        if post_match:
        ...
        if provided_username:
            ...
            info.username = 'Facebook'
            print(f"⚠ Using fallback username: Facebook")
        ...
        # ハッシュタグを生成
        # ベースハッシュタグ（ユーザー名から）
        if info.username == 'Facebook':
            base_hashtag = '#Facebook'
        else:
        ...
        print(f"Error extracting Facebook info: {e}")
        import traceback
        traceback.print_exc()
        
        # フォールバック
        info.url = clean_url(url)
        info.username = provided_username.lstrip('@') if provided_username else 'Facebook'
        info.description = provided_caption or 'Facebook投稿をチェック！'
        
        # ハッシュタグ生成
        base_hashtag = f'#{info.username}' if info.username != 'Facebook' else '#Facebook'
        if provided_hashtags:
            additional_hashtags = format_hashtags(provided_hashtags)
            info.hashtag = f'{base_hashtag} {additional_hashtags}'
        else:
