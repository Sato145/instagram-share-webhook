...
def extract_x_info(url, provided_username='', provided_caption='', provided_hashtags=''):
    """X (Twitter) URLから投稿情報を取得"""
    ...
    try:
        ...
        print(f"Processing X URL: {info.url}")
        
        # ユーザー名を抽出（URLから）
        # パターン: https://twitter.com/username/status/1234567890
        # または: https://x.com/username/status/1234567890
        username = None
        url_match = re.search(r'(?:twitter|x)\.com/([^/]+)/status/', info.url)
        if url_match:
            username = url_match.group(1)
            print(f"✓ Extracted username from URL: {username}")
        
        # ポストIDを抽出
        post_match = re.search(r'/status/(\d+)', info.url)
        if post_match:
        ...
        if provided_username:
            ...
            info.username = 'X'
            print(f"⚠ Using fallback username: X")
        ...
        if provided_caption:
            ...
            info.description = f'{info.username}のポストをチェック！'
        ...
        print(f"Error extracting X info: {e}")
        import traceback
        traceback.print_exc()
        
        # フォールバック
        info.url = clean_url(url)
        info.username = provided_username.lstrip('@') if provided_username else 'X'
        info.description = provided_caption or 'Xポストをチェック！'
        
        # ハッシュタグ生成
        base_hashtag = f'#{info.username}' if info.username != 'X' else '#X'
        if provided_hashtags:
            additional_hashtags = format_hashtags(provided_hashtags)
            info.hashtag = f'{base_hashtag} {additional_hashtags}'
        else:
